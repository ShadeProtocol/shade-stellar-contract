...
pub fn create_invoice_signed(
    ...
    signature_util::invalidate_nonce(env, merchant_address, nonce);
...
pub fn get_invoices(env: &Env, filter: InvoiceFilter) -> Vec<Invoice> {
    ...
    for i in 1..=invoice_count {
            ...
            .get::<_, Invoice>(&DataKey::Invoice(i))
        {
            ...
            if let Some(merchant_address) = &filter.merchant {
...
fn pay_invoice_internal(env: &Env, payer: &Address, invoice_id: u64) {
    let mut invoice = get_invoice(env, invoice_id);
...
pub fn pay_invoice(env: &Env, payer: &Address, invoice_id: u64) {
    payer.require_auth();
    pay_invoice_internal(env, payer, invoice_id);
}
